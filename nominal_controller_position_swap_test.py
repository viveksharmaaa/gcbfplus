...
def animate_position_swap_graph(
    ...
    ax.set_ylabel("y [m]")
    ax.set_title(
        "Nominal Controller Position Exchange with Dynamic Graph Edges"
    )
...
def test(args: Args):
    ...
    # Construct only the environment. No learned policy or checkpoint is used.
    env = make_env(
        env_id=args.env,
    ...
    if not hasattr(env, "u_ref"):
        raise AttributeError(
            "The environment does not expose env.u_ref(graph). "
            "Update nominal_action() to call your nominal controller."
        )

    # Use only the nominal goal-tracking controller.
    # For DoubleIntegrator this is typically a PD controller toward the goal.
    act_fn = jax.jit(env.u_ref)

    print("Controller: nominal env.u_ref only")
    ...
    finish_array = np.asarray(results["finish"], dtype=bool)
    final_finish = finish_array[-1]
    finished_count_over_time = np.sum(finish_array, axis=-1)

    print("Unsafe rollout:", bool(np.any(results["unsafe"])))
    print(
        "Any agent reached a goal at any time:",
        bool(np.any(finish_array)),
    )
    print(
        "All agents simultaneously reached goals:",
        bool(np.any(np.all(finish_array, axis=-1))),
    )
    print(
        "Maximum agents finished simultaneously:",
        int(np.max(finished_count_over_time)),
        "/",
        args.num_agents,
    )
    print(
        "Agents inside goal region at final step:",
        int(np.sum(final_finish)),
        "/",
        args.num_agents,
    )
...
if __name__ == "__main__":
    args = Args(
        ...
        save_animation=True,
        animation_file="nominal_controller_position_swap.gif",
        save_distance_plot=True,
        distance_plot_file="nominal_controller_per_agent_min_distance.png",
        results_file="nominal_controller_position_swap.npz",
        radius_ratio=0.25,  # 0.25 * 4.0 m = 1.0 m swap radius
    )
