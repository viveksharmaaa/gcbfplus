...
# Rollout (b) #(b=num_rollouts, t=time, num_agents, state_dim)
#   └── Time (t)
#         └── Graph G_{b,t}
#               ├── Node 0 (agent or obstacle)
#               ├── Node 1
#               ├── ...
#               ├── Node i  ← your agent
#               └── Edges between nodes
...
class SingleIntegrator(MultiAgentEnv):
    ...
    class EnvState(NamedTuple):
        agent: State #(num_rollouts, time, num_agents, state_dim)
        goal: State #(num_rollouts, time, num_agents, state_dim)
        obstacle: Obstacle
