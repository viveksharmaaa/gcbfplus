...
class GCBFdiffuser(GCBF):
    ...
    def __init__(
        ...
        self.diffusion_key = jr.PRNGKey(seed + 999) #added for diffuser
    ...
    def _graph_with_agent_states(
        ...
        """
        Replace only agent states in a graph and recompute edge features.

        agent_states:
            (n_agents, state_dim)
        """
        new_states = graph.states.at[agent_node_id].set(agent_states)
        return self._env.add_edge_feats(graph, new_states)

    def _joint_dynamics_step(
            self,
            states: Array,
            actions: Array,
    ) -> Array:
        """
        Euler step of the learned environment dynamics.
        ...
        actions:
            (n_agents, action_dim)
        """
        dyn_f, dyn_g = self._env.control_affine_dyn(states)

        # dyn_f: (n_agents, state_dim)
        # dyn_g: (n_agents, state_dim, action_dim)
        xdot = dyn_f + jnp.einsum("anu,au->an", dyn_g, actions)

        return states + self._env.dt * xdot

    def _rollout_joint_controls(
            self,
            graph: GraphsTuple,
            agent_node_id,
        ...
        """
        Roll out one joint multi-agent control sequence.

        control_sequence:
            (horizon, n_agents, action_dim)

        Returns:
            state_traj:
                (horizon + 1, n_agents, state_dim)

            h_traj:
                (horizon + 1, n_agents)
        """

        def evaluate_h(states):
            graph_t = self._graph_with_agent_states(
                graph,
                agent_node_id,
                states,
            )
        ...
        h0 = evaluate_h(initial_states)

        def step(carry, action_t):
            states_t, h_t = carry

            states_next = self._joint_dynamics_step(
                states_t,
                action_t,
            )

            h_next = evaluate_h(states_next)
        ...
        (_, _), (states_future, h_future) = jax.lax.scan(
            step,
            (initial_states, h0),
            control_sequence,
        )
        ...
        return state_traj, h_traj

    def _nominal_control_trajectory(
            self,
            graph: GraphsTuple,
            agent_node_id,
            initial_states: Array,
        ...
        """
        Generate a nominal joint action sequence using u_ref.
        ...
        def step(states_t, _):
            ...
            return states_next, u_t

        _, controls = jax.lax.scan(
            step,
            initial_states,
            xs=None,
            length=horizon,
    ...
    def _diffusion_cost(
            ...
            initial_states: Array,
            controls: Array,
            normalized_nominal_controls: Array,
            cbf_params,
            u_lb: Array,
            u_ub: Array,
            safety_weight: float,
        ...
        """
        Cost of one joint candidate control trajectory.

        controls:
            (horizon, n_agents, action_dim)
        """

        _, h_traj = self._rollout_joint_controls(
            graph=graph,
        ...
        # Same discrete-time CBF condition corresponding to the QP:
        #
        # h_dot + alpha * 0.1 * h >= 0
        #
        h_dot = (h_next - h_now) / self._env.dt

        cbf_residual = (
                h_dot
                + self.alpha * 0.1 * h_now
        )

        # Penalize violation only.
        cbf_violation = jax.nn.relu(
            -cbf_residual
        ) ** 2

        # Also discourage directly entering h < 0 regions.
        unsafe_violation = jax.nn.relu(
            -h_next
        ) ** 2

        # Normalize controls before distance-to-nominal cost.
        controls_norm = (
                2.0
                * (controls - u_lb[None, :, :])
                / (u_ub - u_lb)[None, :, :]
        ...
        nominal_cost = jnp.sum(
            (controls_norm - normalized_nominal_controls) ** 2
        )
        ...
        return nominal_cost + safety_weight * safety_cost

    @ft.partial(jax.jit, static_argnums=(0,))
    def get_diffusion_action(
            self,
            graph: GraphsTuple,
            key: Array,
            cbf_params=None,
        ...
        """
        Diffusion-style replacement for get_qp_action.

        Returns:
            u0: (n_agents, action_dim)

        This is MPC-style:
            sample a safe joint trajectory,
            then apply only the first action.
        """
        assert graph.is_single

        agent_node_mask = graph.node_type == 0
        agent_node_id = mask2index(
            agent_node_mask,
            self.n_agents,
        )

        initial_states = graph.type_states(
            type_idx=0,
        ...
        # --------------------------------------------------------
        nominal_controls = self._nominal_control_trajectory(
            graph=graph,
            agent_node_id=agent_node_id,
            initial_states=initial_states,
            horizon=horizon,
        ...
        nominal_controls_norm = (
                2.0
                * (nominal_controls - u_lb[None, :, :])
                / (u_ub - u_lb)[None, :, :]
                - 1.0
        )
        ...
        # Start around nominal controls instead of all zeros.
        y_current = nominal_controls_norm

        # --------------------------------------------------------
        # Reverse diffusion / weighted sampling
        # --------------------------------------------------------
        for diffusion_idx in range(n_diffuse - 1, 0, -1):
            ...
            sigma = jnp.sqrt(
                1.0 - alphas_bar[diffusion_idx]
            )

            # Candidate normalized control trajectories.
            y_samples = (
                    y_current[None, :, :, :]
                    + sigma * noise
            )
            ...
            # Convert back into actual physical control units.
            u_samples = 0.5 * (
                    y_samples + 1.0
            ) * (
                                u_ub - u_lb
                        )[None, None, :, :] + u_lb[None, None, :, :]

            # Cost each sampled joint trajectory.
            costs = jax.vmap(
                lambda candidate_u: self._diffusion_cost(
                    graph=graph,
                    agent_node_id=agent_node_id,
                    initial_states=initial_states,
                    controls=candidate_u,
                    normalized_nominal_controls=nominal_controls_norm,
                    cbf_params=cbf_params,
                    u_lb=u_lb,
                    u_ub=u_ub,
            ...
            # Lower-cost trajectories receive more weight.
            ...
            y_weighted = jnp.sum(
                weights[:, None, None, None] * y_samples,
                axis=0,
            )

            # Reverse-diffusion update.
            alpha_i = alphas[diffusion_idx]
            alpha_bar_i = alphas_bar[diffusion_idx]

            y_i = y_current * jnp.sqrt(
                alpha_bar_i
            )

            score = (
                            -y_i
                            + jnp.sqrt(alpha_bar_i) * y_weighted
                    ) / (
                            1.0 - alpha_bar_i
                    )
            ...
            y_im1 = (
                            y_i
                            + (1.0 - alpha_bar_i) * score
                    ) / jnp.sqrt(alpha_i)

            if diffusion_idx > 1:
                y_current = y_im1 / jnp.sqrt(
                    alphas_bar[diffusion_idx - 1]
                )
            else:
                y_current = y_im1
        ...
        # MPC: apply only first joint action.
        return best_controls[0]
    #---------------------------------------------------------------------------------------------------------------------------------
    ...
    @ft.partial(jax.jit, static_argnums=(0,)) #added for diffuser
    def get_b_u_diffusion(
            self,
            b_graph: GraphsTuple,
            params,
            key: Array,
        ...
        """
        Batched diffusion-generated action labels.

        Output:
            (batch_size, n_agents, action_dim)
        """
        batch_size = b_graph.n_node.shape[0]
        ...
        b_u_diffusion = jax.vmap(
            lambda graph, sample_key: self.get_diffusion_action(
                graph=graph,
                key=sample_key,
                cbf_params=params,
                horizon=3,
    ...
    def update_nets(self, rollout: Rollout, safe_mask, unsafe_mask):
        ...
        #     b_u_qp.append(jax2np(self.get_b_u_qp(graph, self.cbf_tgt.params)))#[bs=4096, num_agents, action_dim]
        # b_u_qp = tree_merge(b_u_qp)
        #
        # batch_orig = Batch(rollout.graph, safe_mask, unsafe_mask, b_u_qp)

        self.diffusion_key, update_key = jr.split(
            self.diffusion_key
        )
        ...
        for ii in range(n_chunks):
            ...
            update_key, chunk_key = jr.split(
                update_key
            )

            b_u_diffusion.append(
                jax2np(
                    self.get_b_u_diffusion(
                        graph,
                        self.cbf_tgt.params,
                        chunk_key,
                    )
    ...
    def update(self, rollout: Rollout, step: int) -> dict:
        ...
        del rollout
        rollout_batch, safe_mask_batch, unsafe_mask_batch = self.sample_batch(rollout_np, safe_mask, unsafe_mask) #(rollouts*timehorizon, num_agents)
