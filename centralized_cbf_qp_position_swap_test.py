...
def make_position_swap_configuration_(
    num_agents: int,
    area_size: float,
    radius_ratio: float,
):
...
def add_wobble_to_action(
    ...
    """
    Add a small smooth wobble to an action array.

    Args:
        action:
            Action with shape (num_agents, 2). For the double
            integrator, this is acceleration in x and y.
        ...
        amplitude:
            Wobble acceleration amplitude in m/s^2.

        frequency:
            Wobble frequency in Hz.

    Returns:
        Wobbled action with shape (num_agents, 2).
    """
    num_agents = action.shape[0]
    time = step * dt

    # Give each agent a different phase so that all agents do not
    # wobble in exactly the same direction at the same time.
    phase = (
        2.0
        * jnp.pi
        * jnp.arange(num_agents)
    ...
    angle = (
        2.0 * jnp.pi * frequency * time
        + phase
    )

    wobble = amplitude * jnp.stack(
        [
            jnp.sin(angle),
            jnp.cos(angle),
        ],
...
def rollout_position_swap_centralized_qp(
    env,
    controller: CentralizedCBF,
    key,
    rollout_length: int,
    radius_ratio: float,
    ...
    """
    Perfect-state rollout using the centralized CBF-QP controller:
    ...
    for step_idx in trange(
        rollout_length,
        ncols=80,
        desc="Centralized CBF-QP",
    ):
        action, relaxation = qp_fn(graph_true)
#-------------------------------------------------------------------------
        action = add_wobble_to_action(
            action=action,
            step=step_idx,
            dt=env.dt,
            amplitude=0.02,
...
def animate_position_swap_graph(
    ...
    ax.set_ylabel("y [m]")
    ax.set_title(
        "Centralized CBF-QP Position Exchange with Dynamic Graph Edges"
    )
...
def test(args: Args):
    print(
        f"> Running centralized CBF-QP position-swap test: {args}"
    )
    ...
    results = rollout_position_swap_centralized_qp(
        env=env,
    ...
    print("========= CENTRALIZED CBF-QP RESULTS ==========")
    print(
...
if __name__ == "__main__":
    args = Args(
        ...
        save_animation=True,
        animation_file=(
            "centralized_cbf_qp_position_swap.gif"
        ),
        save_distance_plot=True,
        distance_plot_file=(
            "centralized_cbf_qp_per_agent_min_distance.png"
        ),
        results_file=(
            "centralized_cbf_qp_position_swap.npz"
        ),
    ...
    final_speed = np.linalg.norm(
        results["true_states"][-1, :, 2:4],
        axis=-1,
    )

    final_goal_error = np.linalg.norm(
        results["true_states"][-1, :, :2]
        - results["goal_xy"],
        axis=-1,
    )

    print("Final speeds:", final_speed)
    print("Final goal errors:", final_goal_error)
